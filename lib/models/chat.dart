...
class Chat {
  ...
  Chat(this.id, this.type,
      {this.membersId, this.members, this.name, this.messages, this.mostRecent});
  ...
  factory Chat.fromMap(Map<String, dynamic> map) => Chat(
        ...
        membersId: List<Map>.from(map['members'].split(",").map((e) => jsonDecode(e))),
        name: map['name'],
        mostRecent: LocalMessage.empty(),
      );

  static Future<Chat> empty() {
    return Future.value(Chat('', ChatType.individual, mostRecent: LocalMessage.empty()));
  }
}
