...
class _MessageThreadState extends State<MessageThread> {
            ...
            Expanded(
              ...
              builder: (_, state) {
                ...
                return HeaderStatus(
                  ...
                  widget.chat.type == ChatType.individual ? receivers.first.photoUrl : null,
                  widget.chat.type == ChatType.individual ? receivers.first.active! : false,
  ...
  _buildListOfMessages() => ListView.builder(
        ...
        itemBuilder: (__, idx) {
          if (receivers.any((e) => e.id == messages[idx].message.from)) {
            ...
            final receiver = receivers.firstWhere((e) => e.id == messages[idx].message.from);

            final String color =
                widget.chat.membersId?.firstWhere((e) => e.containsKey(receiver.id))?.values?.first;
            return Padding(
            ...
            return Padding(
                padding: const EdgeInsets.only(bottom: 8.0), child: SenderMessage(messages[idx]));
          }
        },
  ...
  _buildMessageInput(BuildContext context) {
    final _border = OutlineInputBorder(
      ...
      borderSide:
          isLightTheme(context) ? BorderSide.none : BorderSide(color: Colors.grey.withOpacity(0.3)),
    );

    return Focus(
      onFocusChange: (focus) {
        if (_startTypingTimer == null || (_startTypingTimer != null && focus)) return;
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          filled: true,
          fillColor: isLightTheme(context) ? kPrimary.withOpacity(0.1) : kBubbleDark,
  ...
  void _dispatchTyping(Typing event) {
    ...
    final typings = receivers
        .map((e) => TypingEvent(chatId: chatid, from: widget.me.id, to: e.id, event: event))
        .toList(); //create all typing events to users in group

    widget.typingNotificationBloc.add(TypingNotificationEvent.onTypingEventSent(typings));
  }
  ...
  void _sendTypingNotification(String text) {
    ...
    _startTypingTimer = Timer(Duration(seconds: 5), () {}); //send one event every 5 seconds

    _stopTypingTimer = Timer(Duration(seconds: 6), () => _dispatchTyping(Typing.stop));
  }
