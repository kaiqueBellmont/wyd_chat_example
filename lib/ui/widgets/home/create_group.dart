...
class _CreateGroupState extends State<CreateGroup> {
  ...
  void initState() {
    ...
    _messageGroupBloc.stream.listen((state) async {
      if (state is MessageGroupCreatedSuccess) {
        state.group.members.removeWhere((e) => e == widget._me.id); //remove myself from ids
        ...
        await _chatsCubit.viewModel.createNewChat(chat); //save in local database
                  ...
                  state.isEmpty
                      ...
                      : Container(
                          ...
                          child: ListView.builder(
                            itemBuilder: (__, idx) => _selectedUsersListItem(selectedUsers[idx]),
                            itemCount: selectedUsers.length,
                            scrollDirection: Axis.horizontal,
                            padding: EdgeInsets.symmetric(horizontal: 16.0),
                          ),
                        ),
                  Expanded(child: _buildList(widget._activeUsers)) //show list of active users
                ],
              ),
            ),
          );
            ...
            child: Stack(
              children: [
                  ...
                  child: Align(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          color: isLightTheme(context) ? Colors.black54 : kAppBarDark),
                      child: Icon(
                        Icons.close_rounded,
                        color: Colors.white,
                        size: 12.0,
                ...
                Align(
                  ...
                  child: Text(
                    user.username.split(' ').first, //set user name to first part only
                    softWrap: false,
  ...
  _buildList(List<User> users) => ListView.separated(
      ...
      itemBuilder: (BuildContext context, indx) => GestureDetector(
            ...
            onTap: () {
              if (selectedUsers.any((element) => element.id == users[indx].id)) {
