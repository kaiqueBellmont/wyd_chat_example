...
class _ChatsState extends State<Chats> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ChatsCubit, List<Chat>>(builder: (__, chats) {
      ...
      context.read<TypingNotificationBloc>().add(TypingNotificationEvent.onSubscribed(widget.user,
        ...
        itemBuilder: (_, indx) => GestureDetector(
              ...
              onTap: () async {
                await this
                    .widget
                    .router
                    .onShowMessageThread(context, chats[indx].members, widget.user, chats[indx]);
  ...
  _chatItem(Chat chat) => ListTile(
        ...
        leading: ProfileImage(
          ...
          online: chat.type == ChatType.individual ? chat.members?.first.online : false,
        ),
        title: Text(
          chat.type == ChatType.individual ? chat.members?.first.username ?? '' : chat.name ?? '',
        ...
        subtitle:
            BlocBuilder<TypingNotificationBloc, TypingNotificationState>(builder: (__, state) {
          if (state is TypingNotificationReceivedSuccess &&
              state.event.event == Typing.start &&
              state.event.chatId == chat.id) this.typingEvents.add(state.event.chatId);

          if (state is TypingNotificationReceivedSuccess &&
              state.event.event == Typing.stop &&
              state.event.chatId == chat.id) this.typingEvents.remove(state.event.chatId);

          if (this.typingEvents.contains(chat.id)) {
            switch (chat.type) {
              case ChatType.group:
                final st = state as TypingNotificationReceivedSuccess;
                final username = chat.members?.firstWhere((e) => e.id == st.event.from).username;
    ...
    context.read<MessageGroupBloc>().stream.listen((state) async {
      if (state is MessageGroupReceived) {
        ...
        final chat = Chat(group.id!, ChatType.group,
            name: group.name, membersId: membersId, mostRecent: null);
